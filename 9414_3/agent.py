...
class agent():
    ...
    def shift(self, x, y, target_x, target_y, act=''):
        ...
        if (act):
            temp = self.pat.change(self.sta.row, self.sta.col, x, y)
        else:
        ...
        for p in temp:
            ...
            for a_s in action_str:
                ...
                self.comm.read_view()
        if (self.m.map[self.sta.row][self.sta.col] == '$'):
            self.sta.have_treasure =True
            self.m.map[self.sta.row][self.sta.col] = ' '
        elif (self.m.map[self.sta.row][self.sta.col] == 'd'):
            self.sta.num_dynamites_held += 1
            self.m.map[self.sta.row][self.sta.col] = ' '
        elif (self.m.map[self.sta.row][self.sta.col] == 'k'):
            self.sta.have_key += 1
            self.m.map[self.sta.row][self.sta.col] = ' '
        elif (self.m.map[self.sta.row][self.sta.col] == 'a'):
            self.sta.have_axe += 1
            self.m.map[self.sta.row][self.sta.col] = ' '
        if bool(act):
        ...
        try:
            while(True):
                ...
                if (self.m.map[self.sta.row][self.sta.col] == '~'):
                    ...
                    for gjy in final_decision:
                        for p in gjy[1:]:
                            if ( self.sta.have_axe and p[2] == 'T' and self.eva.need_to_cut(p[0],p[1])):
                                final_decision_1 = gjy[0]
                                final_decision_2 = [p[0],p[1]]
                                break
                            if (p[2] == '^' and not final_decision_2):
                                final_decision_1 = gjy[0]
                                final_decision_2=[]
                                self.sta.refresh_place=True
                                break
                            if (p[2] in {'d', '$', 'k', 'a'} and not final_decision_2):
                                final_decision_1 = [p[0], p[1]]
                                final_decision_2 = []
                                break
                            if (self.sta.have_treasure and gjy[0]==[self.m.begin]):
