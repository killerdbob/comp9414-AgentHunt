...
def simulate(x,y):
    ...
    option=[]
    if(map[x][y] == ' '):
        g_option=get_all_choice(x,y)
    if (map[x][y] == '~'):
        o_option=get_all_choice(x,y)
...
try:
    while(not game_won):
        ...
        if (map[row][col] ==' '):
            off_x = 0
            off_y = 0
            main_option=get_all_choice(row,col)
