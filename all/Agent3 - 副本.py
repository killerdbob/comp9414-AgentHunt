...
try:
    while(not game_won):
        ...
        if (map[row][col] ==' ' and not have_treasure):
            off_x = 0
            off_y = 0
            main_option=find_ground_options(row,col)
            for opt in main_option:
                for o in opt[1:]:
                    if ( have_axe and o[2] == 'T' and is_value_tree(o[0],o[1])):
                        if ((main_temp1 and manhattan_dist(row, col, main_temp1[0], main_temp1[1])
                            ...
                            main_temp1 = opt[0]
                            main_temp2 = [o[0],o[1]]
                    if (o[2] == 'm' and not main_temp2):
                        if ((main_temp1 and manhattan_dist(row, col, main_temp1[0], main_temp1[1])
                            > manhattan_dist(row, col, opt[0][0], opt[0][1])) or not main_temp1):
                            main_temp1 = opt[0]
                            g_think=True
                    if (o[2] in {'d', 'a', '$', 'k'} and not main_temp2):
                        if ((main_temp1 and manhattan_dist(row, col, main_temp1[0], main_temp1[1])
                            > manhattan_dist(row, col, opt[0][0], opt[0][1])) or not main_temp1):
                            main_temp1 = [o[0], o[1]]
            if(main_temp1 and not have_treasure):
                if(main_temp2 ):
