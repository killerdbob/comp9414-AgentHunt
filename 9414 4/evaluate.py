...
class eval():
    ...
    def need_to_cut(self,x, y):#判断树的价值
        num = 0
        do_calcant = False
        for i in range(25):
            if ((self.m.map[x + i//5][y + i%5] in {'d', 'k', 'a', '$', '?'}) and not do_calcant):
                num += 1
        return num
    def do_calc(self,x, y, x0, y0, layer):#判断墙的价值，递归炸墙，判断墙是否有炸的价值
        ...
        if ([x, y - 1] not in self.d_temp_wall and self.m.map[x][y - 1] not in {'.', '~'}):
            if (self.m.map[x][y - 1] == '*' and self.sta.num_dynamites_held >= 2):
                ...
                if (self.do_calc(x, y - 1, x0, y0,self.sta.num_dynamites_held)):
                    self.sta.num_dynamites_held += 1#恢复堆栈
                    return True
                self.sta.num_dynamites_held += 1#恢复堆栈
    ...
    def change_goal(self,x, y):#初始化calculation
        self.path=[]
        self.temp_path=[]
        self.d_tree=[]
        self.d_wall=[]
    ...
    def calculation(self,x, y):#递归查找最优解
