...
def find_path(x,y,dx,dy):
    ...
    def recursive_path(cx,cy):
        ...
        for _ in range(4):
            a_star = []
            temp = []
            if(([cx-1,cy]==[dx,dy] or map[cx-1][cy] == current_state) and [cx-1,cy] not in used_path):
                a_star.append([cx-1,cy,manhattan_dist(x,y,cx-1,cy)+manhattan_dist(dx,dy,cx-1,cy)])
            if(([cx,cy+1]==[dx,dy] or map[cx][cy+1] ==current_state) and [cx,cy+1] not in used_path):
                a_star.append([cx,cy+1,manhattan_dist(x,y,cx,cy+1)+manhattan_dist(dx,dy,cx,cy+1)])
            if(([cx+1,cy]==[dx,dy] or map[cx+1][cy] ==current_state) and [cx+1,cy] not in used_path):
                a_star.append([cx+1,cy,manhattan_dist(x,y,cx+1,cy)+manhattan_dist(dx,dy,cx+1,cy)])
            if(([cx,cy-1]==[dx,dy] or map[cx][cy-1] ==current_state) and [cx,cy-1] not in used_path):
                a_star.append([cx,cy-1,manhattan_dist(x,y,cx,cy-1)+manhattan_dist(dx,dy,cx,cy-1)])
...
def simulate(x,y):
    ...
    if(map[x][y]==' '):#pick treasure
        ...
        if(temp2):
            ...
            if(map[temp2[0]][temp2[1]]=='$'):
                have_treasure-=1
            recover_mist(mist)
    if (map[x][y] == ' '):  # cut a tree to find a road
        temp1 = []
        temp2 = []
        if(have_axe and have_raft):
            for opt in g_option:
                for o in opt[1:]:
    ...
    if (map[x][y] == '~'):#sailing, looking for ocean mist
        temp1 = []
        for opt in o_option:
            for o in opt[1:]:
    ...
    if (map[x][y] == '~'):  # find a new continent
        ...
        for sc in search_continent:
            ...
            if (temp2 ):
                #print('search_continent:', search_continent)
                mist2 = clear_mist(temp2[0][0], temp2[0][1])
                temp_path.append(temp2)
            ...
            for o in opt[1:]:
                ...
                if (temp1 and temp2 ):
                    ...
                    if(o[2] == 'd' ):
                        ...
                        map[temp2[0]][temp2[1]]=' '
                        num_dynamites_held +=1
                        temp_mark = ground_evaluate(temp2[0],temp2[1])-1000#be cautious while landing
                        u_wall=used_wall
                        u_tree=used_tree
                        used_wall=[]
                        used_tree=[]
    ...
    if (map[x][y] == ' '):  # blow up door
        if(not have_key and num_dynamites_held):
            temp1 = []
            temp2 = []
            for opt in g_option:
                for o in opt[1:]:
                    if (o[2] == '-' and is_value(o[0],o[1])):
                        temp1 = opt[0]
                        temp2 = [o[0], o[1]]
                        if(temp1 and temp2 ):
                            mist1=clear_mist(temp1[0], temp1[1])
    ...
    if (map[x][y] == ' '):  # blow up tree
        if(not have_axe and num_dynamites_held):
            temp1 = []
            temp2 = []
            for opt in g_option:
                for o in opt[1:]:
                    if (o[2] == 'T' and is_value_tree(o[0],o[1]) and [o[0], o[1]] not in used_tree):
                        used_tree.append([o[0], o[1]])
                        temp1 = opt[0]
                        temp2 = [o[0], o[1]]
                        if(temp1 and temp2 ):
                            mist1=clear_mist(temp1[0], temp1[1])
