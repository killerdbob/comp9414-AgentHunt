...
def ground_evaluate(x,y):
    ...
    if(map[x][y]==' '):
        ...
        mark=len(dynamites)*100+num_dynamites_held*200+((axe or have_axe) and len(tree)>=1)*20\
             ...
             +key*10+len(explored_mist)*3+len(mist)*2
    return mark
...
def ocean_evaluate(x,y):
    ...
    if(map[x][y]=='~'):
        ...
        mark = len(mist) + len(explored_mist) * 1.2 + num_dynamites_held * 200 +bool(have_axe) * 20 \
               + bool(have_raft) * 4 + len(mist)*2 + len(explored_mist) * 3 + bool(have_key) * 20
    return mark
...
def simulate(x,y):
    ...
    if (game_won):
        return
    if (map[x][y] == ' ' and have_treasure):  # find the treasure and go home
        if([[center_x,center_y]] in g_option):
    ...
    if (map[x][y] == ' '):  # explore ground mist
        temp1 = []
        for opt in g_option:
            for o in opt[1:]:
                if(o[2] == 'm' and map[opt[0][0]][opt[0][1]]==' ' and opt[0] not in used_ground_mist):
        ...
        if(temp1):
            ...
            if(temp_mark > simulate_mark or (temp_mark == simulate_mark and len(temp_path)<len(best_path))):
                best_path=[i for i in temp_path]
                simulate_mark=temp_mark
            simulate(temp1[0],temp1[1])
            temp_path.pop()
            recover_mist(mist)

    if (map[x][y] == ' ' and have_key):  # open ground door
        temp1 = []
        temp2 = []
        for opt in g_option:
            for o in opt[1:]:
    ...
    if (map[x][y] == '~' and have_key):  # open ocean door
        temp1 = []
        temp2 = []
        for opt in o_option:
            for o in opt[1:]:
    ...
    if (map[x][y] == ' ' and have_axe):  # cut a tree to find a road
        temp1 = []
        temp2 = []
        for opt in g_option:
            for o in opt[1:]:
                if (is_value_tree(o[0],o[1]) and [o[0], o[1]] not in used_tree and o[2] == 'T'):
                    temp1 = opt[0]
                    temp2 = [o[0], o[1]]
                    break
        if(temp1):
    ...
    if (map[x][y] == ' ' and have_raft):  # prepare for sailing
        search_ocean = []
        for opt in g_option:
            for o in opt[1:]:
        ...
        for sc in search_ocean:
            if (sc not in temp_path):
                ...
                continue
            # print('search_continent:', search_continent)
            mist1 = clear_mist(temp2[0][0], temp2[0][1])
            mist2 = clear_mist(temp2[1][0], temp2[1][1])
            temp_path.append(temp2)
            reduce_mark -=500# be cautious while sailing
    ...
    if (map[x][y] == '~' and have_raft):#sailing, looking for ocean mist
        temp1 = []
        for opt in o_option:
            for o in opt[1:]:
                if(o[2] == 'm' and map[opt[0][0]][opt[0][1]]=='~' and opt[0] not in used_ocean_mist):
                    if((temp1 and manhattan_dist(x,y,temp1[0],temp1[1])>manhattan_dist(x,y,opt[0][0],opt[0][1]))
                       or not temp1):
                        temp1=opt[0]
        if(temp1 ):
    ...
    if (map[x][y] == '~' and have_key):  # open ocean door
        ...
        for opt in o_option:
            for o in opt[1:]:
                if(o[2] == '-'):
                    if((temp1 and manhattan_dist(x,y,temp1[0],temp1[1])>manhattan_dist(x,y,opt[0][0],opt[0][1]))or not temp1):
                        temp1=opt[0]
                        temp2=[o[0],o[1]]
        if (temp1 and temp2):
            mist1=clear_mist(temp1[0], temp1[1])
            ...
            if(map[temp2[0]][temp2[1]]=='-'):
                map[temp2[0]][temp2[1]] = ' '
                temp_path.append([temp1,temp2,'U'])
            island_ocean_index.append([[temp2[0], temp2[1]],' '])
            temp_mark = ocean_evaluate(x,y)
            if(temp_mark > simulate_mark or (temp_mark == simulate_mark and len(temp_path)<len(best_path))):
                best_path=[i for i in temp_path]
                simulate_mark = temp_mark
    ...
    if (map[x][y] == '~'):  # find a new continent
        ...
        for sc in search_continent:
            if (sc not in temp_path):
                #print('search_continent:', search_continent)
                mist2 = clear_mist(sc[0][0], sc[0][1])
                temp_path.append(sc)
                reduce_mark -=1000#be cautious while landing
                temp_mark = ocean_evaluate(sc[0][0],sc[0][1])
                u_tree=deepcopy(used_tree)
                u_wall=deepcopy(used_wall)
                used_wall=[]
                used_tree=[]
                temp_raft = have_raft
                have_raft = 0
            ...
            for o in opt[1:]:
                ...
                if (temp1 and temp2 ):
                    if(o[2] == 'd' ):
                        ...
                        num_dynamites_held +=1
                        reduce_mark -= 1000
                        temp_mark = ground_evaluate(temp2[0],temp2[1])#be cautious while landing
                        u_wall=deepcopy(used_wall)
                        u_tree=deepcopy(used_tree)
                        temp_raft = have_raft
                        have_raft = 0
    ...
    if (map[x][y] == ' ' and num_dynamites_held):  # blow up wall
        for opt in g_option:
            for o in opt[1:]:
    ...
    if (map[x][y] == '~' and num_dynamites_held):  # blow up ocean wall
        for opt in o_option:
            for o in opt[1:]:
                if (o[2] == '*' and is_value(o[0],o[1]) and [o[0], o[1]] not in used_wall):
                    temp1 = opt[0]
                    ...
                    if(temp1 and temp2 ):
                        ...
                        num_dynamites_held -= 1
                        temp_path.append([temp1,temp2,'B'])
                        temp_mark = ocean_evaluate(temp2[0],temp2[1])
                        if(temp_mark > simulate_mark or (temp_mark == simulate_mark and len(temp_path)<len(best_path))):
                            best_path=[i for i in temp_path]
                            simulate_mark = temp_mark
                        simulate(temp2[0],temp2[1])
    ...
    if (map[x][y] == ' ' and have_axe and not have_raft):  # make a boat
        temp1 = []
        temp2 = []
        for opt in g_option:
            for o in opt[1:]:
    ...
    #print('temp_path:', temp_path, 'simulate_mark:', simulate_mark)
def print_map():
    os.system('clear')
...
def move(x,y,dx,dy,act=''):
    ...
    if( act):
        ...
        #print('find_path:',find_path(row, col, x, y))
    else:
        #print('map[row][col],map[dx][dy]:',map[row][col],map[dx][dy])
        if (map[row][col] == '~' and map[dx][dy] in {' ','d','$','a','k','-','T','*'} ):
            have_raft = 0
        temp = find_path(row,col, dx, dy)
        #print('find_path:',find_path(row, col, dx, dy))
    for p in temp:
...
try:
    while(not game_won):
        ...
        if (map[row][col] ==' ' and have_raft):
            search_ocean=[]
            for i in find_all_oceans(row, col):
                search_ocean.append(i[0])
                break
            if(search_ocean):
                move(row,col,search_ocean[0][1][0],search_ocean[0][1][1])
